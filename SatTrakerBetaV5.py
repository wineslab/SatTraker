...
if sys.platform == "win32":
    ...
    win32com = None  # placeholder for non-Windows systems
import pwi4_client

event = (
    threading.Event()
)  # Used with satmonitor and exitprog to prevent an error when exiting
    ...
    def get_x_y(img, roibox, imageroi):
        ...
        if trackSettings.trackingtype == "Bright":
            ...
            thresh = cv2.threshold(
                blurred, float(trackSettings.minbright), 255, cv2.THRESH_BINARY
            )[1]
...
class buttons:
    def __init__(self, master):
        ...
        # Hide Lat/Lon
        self.hidell = IntVar()
        self.hideLatlon = Checkbutton(
            self.bottomframe,
            text="Hide Latitude/Longitude",
            variable=self.hidell,
            command=self.hide_latLong,
        )
        ...
        self.satDrop = OptionMenu(
            self.bottomframe, self.selectedsat, *self.satlist, command=self.sat_changed
        )
        ...
        self.startButton = Button(
            self.bottomframe, text="Start Camera", command=self.set_img_collect
        )
        self.startButton.grid(row=1, column=0)
        self.startButton2 = Button(
            self.bottomframe, text="Camera Calibration", command=self.start_calibration
        )
        self.startButton2.grid(row=4, column=0)
        self.startButton3 = Button(
            self.bottomframe, text="Set Center Point", command=self.set_center
        )
        self.startButton3.grid(row=4, column=1)
        self.startButton4 = Button(
            self.bottomframe,
            text="Start Tracking Satellite",
            command=self.start_sat_track,
        )
        self.startButton4.grid(row=8, column=1)
        self.startButton5 = Button(
            self.bottomframe, text="Connect Scope", command=self.set_tracking
        )
        self.startButton5.grid(row=1, column=1)
        self.startButton6 = Button(
            self.bottomframe, text="Reset Crosshair", command=self.set_crosshair
        )
        self.startButton6.grid(row=4, column=2)
        self.ComLabel = Label(self.bottomframe, text="COM Port")
        self.ComLabel.grid(row=2, column=0)
        self.recordv = Checkbutton(
            self.bottomframe, text="Record Video", variable=self.recordvideo
        ...
        self.CalspeedLabel = Label(
            self.bottomframe, text="Calibration Speed \n(degree/second)"
        )
        ...
        self.fileMenu.add_command(
            label="Exit and Save Configuration", command=self.exitProg
        )

        self.telescopeMenu = Menu(self.menu, tearoff="off")
        self.menu.add_cascade(label="Telescope Type", menu=self.telescopeMenu)
        self.telescopeMenu.add_command(
            label="LX200 Classic Alt/Az", command=self.setLX200AltAz
        )
        self.telescopeMenu.add_command(
            label="LX200 Classic Equatorial", command=self.setLX200Eq
        )
        self.telescopeMenu.add_command(label="ASCOM Alt/Az", command=self.setASCOMAltAz)
        self.telescopeMenu.add_command(
            label="ASCOM Equatorial", command=self.setASCOMEq
        )
        self.telescopeMenu.add_command(
            label="LX200 Autostar Alt/Az", command=self.setAutostarAltAz
        )
        self.telescopeMenu.add_command(
            label="PlaneWave Alt/Az", command=self.setPlaneWaveAltAz
        )

        self.cameraMenu = Menu(self.menu, tearoff="off")
        self.menu.add_cascade(label="Camera Type", menu=self.cameraMenu)
        self.cameraMenu.add_command(
            label="Windows Camera", command=self.setWindowsCamera
        )
        self.cameraMenu.add_command(label="ASCOM Camera", command=self.setASCOMCamera)
        self.cameraMenu.add_command(label="ZWO ASI Camera", command=self.setZWOASICamera)

        self.trackingMenu = Menu(self.menu)
        self.menu.add_cascade(label="Tracking Type", menu=self.trackingMenu)
        self.trackingMenu.add_command(
            label="Feature Tracking", command=self.setFeatureTrack
        )
        self.trackingMenu.add_command(
            label="Brightness Tracking", command=self.setBrightTrack
        )
        ...
        self.imageMenu.add_command(
            label="Horizontal Flip", command=self.setHorizontalFlip
        )
        self.imageMenu.add_command(
            label="Vertical and Horizontal Flip", command=self.setVerticalHorizontalFlip
        )
        self.imageMenu.add_command(
            label="Rotate Image 0 Degrees", command=self.set0Rotate
        )
        self.imageMenu.add_command(
            label="Rotate Image 90 Degrees", command=self.setPos90Rotate
        )
        self.imageMenu.add_command(
            label="Rotate Image -90 Degrees", command=self.setNeg90Rotate
        )
        self.imageMenu.add_command(
            label="Rotate Image 180 Degrees", command=self.set180Rotate
        )
    ...
    def setZWOASICamera(self):
        trackSettings.cameratype = "ZWO_ASI"
    ...
    def search(self, key):
        ...
        for idx, name in enumerate(trackSettings.satnamelist):
            if searchterm in name:
                ...
                self.monitorsat = ephem.readtle(
                    trackSettings.tlelist[trackSettings.satselection][0],
                    trackSettings.tlelist[trackSettings.satselection][1],
                    trackSettings.tlelist[trackSettings.satselection][2],
                )
    ...
    def filePicker(self):
        trackSettings.orbitFile = filedialog.askopenfilename(
            initialdir=".",
    ...
    def hide_latLong(self):
        if self.entryLon.cget("show") == "":
            ...
            answer = messagebox.askquestion(
                ...
                "Are you sure you want to show latitude and longitude?",
            )
            if answer == "yes":
                self.entryLon.config(show="")
                self.entryLat.config(show="")
    ...
    def start_sat_track(self):
        if trackSettings.trackingsat is False:
            ...
            if (
                ...
                and trackSettings.trackingsat is False
            ):
        ...
        if trackSettings.satelliteselected is False:
            ...
            self.textbox.insert(
                END, "Select Satellite From The Drop Down Menu First!\n"
            )
            self.textbox.see("end")
        if (
            trackSettings.tracking is True
            and self.collect_images is True
            and trackSettings.trackingsat is True
            and trackSettings.fileSelected is True
            and trackSettings.satelliteselected is True
        ):
            self.observer = ephem.Observer()
            self.observer.lat = str(self.entryLat.get())
            self.observer.lon = str(self.entryLon.get())
            ...
            if firstslew is True:
                ...
                if trackSettings.telescopetype == "PlaneWave":
                    ...
                    if (
                        ...
                        self.PWscope.mount_follow_tle(
                            trackSettings.tlelist[trackSettings.satselection][0],
                            trackSettings.tlelist[trackSettings.satselection][1],
                            trackSettings.tlelist[trackSettings.satselection][2],
                        )
                        azrate = math.degrees(self.radaz2 - self.radaz)
                        headingeast = (math.degrees(self.radaz2) + 360) - math.degrees(
                            self.radaz
                        )
                        headingwest = math.degrees(self.radaz2) - (
                            math.degrees(self.radaz) + 360
                        )
                ...
                if trackSettings.telescopetype == "ASCOM":
                    ...
                    if (
                        ...
                        self.tel.SlewToAltAz(
                            math.degrees(self.radaz2), math.degrees(self.radalt2)
                        )
                    ...
                    if (
                        ...
                        rarate = (
                            -1
                            * (math.degrees(self.radra2 - self.radra))
                            * math.cos(self.raddec2)
                        )
                        decrate = math.degrees(self.raddec2 - self.raddec)
                        # self.tel.Tracking = False
                        self.tel.SlewToCoordinates(
                            (math.degrees(self.radra2) / 15), math.degrees(self.raddec2)
                        )
                ...
                if trackSettings.telescopetype == "LX200":
                    if trackSettings.mounttype == "AltAz":
                        ...
                        if i > 100:
                            ...
                            self.textbox.insert(
                                END,
                                str("Distance from target: " + str(totaldiff) + "\n"),
                            )
                            self.textbox.see("end")
                            self.lasttotaldiff = totaldiff
                    ...
                    if trackSettings.mounttype == "Eq":
                        ...
                        print(math.degrees(totaldiff))
                        self.textbox.insert(
                            END, str("Distance from target: " + str(totaldiff) + "\n")
                        )
                        self.textbox.see("end")
                ...
                if trackSettings.telescopetype == "PlaneWave":
                    ...
                    if (
                        ...
                        if self.dnow > self.dlast:
                            ...
                            objectvertical = -1 * (
                                (self.targetY - trackSettings.mainviewY)
                                * trackSettings.imagescale
                            )
                            objecthorizontal = (
                                self.targetX - trackSettings.mainviewX
                            ) * trackSettings.imagescale
                            objectangle = (
                                math.degrees(
                                    math.atan2(objectvertical, objecthorizontal)
                                )
                                - 90
                            )
                            objectangle2 = math.degrees(
                                math.atan2(objectvertical, objecthorizontal)
                            )
                            objectdistance = math.sqrt(
                                (objecthorizontal**2)
                                + (objectvertical**2)
                                - 2
                                * (
                                    objectvertical
                                    * objecthorizontal
                                    * math.cos(math.radians(objectangle))
                                )
                            )
                            try:
                                objectalt = 90 - math.degrees(
                                    math.acos(
                                        math.cos(math.radians(objectdistance))
                                        * math.cos(math.radians(90 - currentaltdegrees))
                                        + math.sin(math.radians(objectdistance))
                                        * math.sin(math.radians(90 - currentaltdegrees))
                                        * math.cos(math.radians(objectangle))
                                    )
                                )
                                diffinaz = math.degrees(
                                    math.acos(
                                        (
                                            math.cos(math.radians(objectdistance))
                                            - math.cos(
                                                math.radians(90 - currentaltdegrees)
                                            )
                                            * math.cos(math.radians(90 - objectalt))
                                        )
                                        / (
                                            math.sin(
                                                math.radians(90 - currentaltdegrees)
                                            )
                                            * math.sin(math.radians(90 - objectalt))
                                        )
                                    )
                                )
                                if math.fabs(objectangle2) > 90:
                                    diffinaz = -1 * diffinaz
                                altdiff = math.degrees(
                                    math.radians(objectalt) - currentalt
                                )
                                azdiff = diffinaz
                                totaldiff = math.sqrt(altdiff**2 + azdiff**2)
                                self.observer.date = d + datetime.timedelta(seconds=1)
                                ...
                                azrate = math.degrees(self.radaz2 - self.radaz)
                                headingeast = (
                                    math.degrees(self.radaz2) + 360
                                ) - math.degrees(self.radaz)
                                headingwest = math.degrees(self.radaz2) - (
                                    math.degrees(self.radaz) + 360
                                )
                                # Check for north meridian
                ...
                if trackSettings.telescopetype == "ASCOM":
                    ...
                    if (
                        ...
                        if self.dnow > self.dlast:
                            ...
                            try:
                                objectdec = 90 - math.degrees(
                                    math.acos(
                                        math.cos(math.radians(objectdistance))
                                        * math.cos(math.radians(90 - currentdecdegrees))
                                        + math.sin(math.radians(objectdistance))
                                        * math.sin(math.radians(90 - currentdecdegrees))
                                        * math.cos(math.radians(objectangle))
                                    )
                                )
                                diffinra = math.degrees(
                                    math.acos(
                                        (
                                            math.cos(math.radians(objectdistance))
                                            - math.cos(
                                                math.radians(90 - currentdecdegrees)
                                            )
                                            * math.cos(math.radians(90 - objectdec))
                                        )
                                        / (
                                            math.sin(
                                                math.radians(90 - currentdecdegrees)
                                            )
                                            * math.sin(math.radians(90 - objectdec))
                                        )
                                    )
                                )
                                if math.fabs(objectangle2) > 90:
                    ...
                    if (
                        ...
                        if self.dnow > self.dlast:
                            ...
                            try:
                                ...
                                self.textbox.see("end")
                                self.ser.write(
                                    str.encode(str(":RA" + str(azrate) + "#"))
                                )
                                self.ser.write(
                                    str.encode(str(":RE" + str(altrate) + "#"))
                                )
                                self.ser.write(str.encode(":Me#"))
                                self.ser.write(str.encode(":Mn#"))
                                self.diffazlast = azdiff
                        ...
                        if self.dnow > self.dlast:
                            ...
                            try:
                                objectalt = 90 - math.degrees(
                                    math.acos(
                                        math.cos(math.radians(objectdistance))
                                        * math.cos(math.radians(90 - self.telalt))
                                        + math.sin(math.radians(objectdistance))
                                        * math.sin(math.radians(90 - self.telalt))
                                        * math.cos(math.radians(objectangle))
                                    )
                                )
                                diffinaz = math.degrees(
                                ...
                                self.textbox.insert(
                                    END,
                                    str(
                                        "Distance from target: " + str(totaldiff) + "\n"
                                    ),
                                )
                                self.textbox.see("end")
                                self.lasttotaldiff = totaldiff
                        ...
                        if self.dnow > self.dlast:
                            ...
                            try:
                                objectalt = 90 - math.degrees(
                                    math.acos(
                                        math.cos(math.radians(objectdistance))
                                        * math.cos(math.radians(90 - self.teldec))
                                        + math.sin(math.radians(objectdistance))
                                        * math.sin(math.radians(90 - self.teldec))
                                        * math.cos(math.radians(objectangle))
                                    )
                                )
                                diffinaz = math.degrees(
    ...
    def set_img_collect(self):
        if self.collect_images is False:
            ...
            elif trackSettings.cameratype == "ZWO_ASI":
        ...
        if trackSettings.tracking is False:
            ...
            if trackSettings.telescopetype == "LX200":
                try:
                    ...
                    self.textbox.insert(
                        END, str("Failed to connect on " + str(self.comport) + "\n")
                    )
                    self.textbox.see("end")
                ...
                if self.tel.Connected:
                    ...
                    if self.tel.Connected:
                        ...
                        if axis is False or axis2 is False:
                            print(
                                "This scope cannot use the MoveAxis method, aborting."
                            )
                            self.textbox.insert(
                                END,
                                str(
                            ...
                            self.axis0rate = float(
                                self.tel.AxisRates(0).Item(1).Maximum
                            )
                            self.axis1rate = float(
                                self.tel.AxisRates(1).Item(1).Maximum
                            )
                        ...
                        self.textbox.insert(
                            END,
                            str("Unable to connect to telescope, expect exception.\n"),
                        )
                        self.textbox.see("end")
        else:
            ...
            if trackSettings.telescopetype == "LX200" and self.serialconnected is True:
                ...
                trackSettings.telescopetype == "Autostar"
                and self.serialconnected is True
            ):
                self.ser.write(str.encode(":Q#"))
                # self.ser.write(str.encode(':U#'))
                self.ser.close()
    ...
    def rad_to_sexagesimal_alt(self):
        ...
        self.alt_s = (
            ((abs(self.altdeg) - abs(self.alt_d)) * 60) - abs(self.alt_m)
        ) * 60
    ...
    def rad_to_sexagesimal_ra(self):
        ...
        self.dec_s = (
            ((abs(self.decdeg) - abs(self.dec_d)) * 60) - abs(self.dec_m)
        ) * 60
    ...
    def set_calibration(self):
        ...
        if (
            trackSettings.telescopetype == "ASCOM"
            or trackSettings.telescopetype == "PlaneWave"
        ):
            try:
                ...
                if float(trackSettings.calspeed) == 0.0:
                    ...
                    self.textbox.insert(
                        END, str("Calibration speed needs to be more than zero!")
                    )
                    trackSettings.calibratestart = False
            except Exception as e:
                speedset = False
        ...
        if (
            trackSettings.tracking is True
            and self.collect_images is True
            and trackSettings.objectfollow is True
            and trackSettings.calibratestart is True
            and speedset is True
        ):
            if trackSettings.telescopetype == "PlaneWave":
                if trackSettings.mounttype == "AltAz":
                    getstatus = self.PWscope.status()
    ...
    def separation_between_coordinates(self):
        self.separation = math.degrees(
            math.acos(
                math.sin(self.Y1) * math.sin(self.Y2)
                + math.cos(self.Y1) * math.cos(self.Y2) * math.cos(self.X1 - self.X2)
            )
        )
    ...
    def _on_mousewheel(self, event):
        ...
        self.textbox.insert(
            END, str("Tracking box size: " + str(trackSettings.boxSize) + "\n")
        )
        self.textbox.see("end")
    ...
    def left_click(self, event):
        if trackSettings.setcenter is True:
            ...
            self.imageroi = self.img[
                self.mousebox[0][1] : self.mousebox[1][1],
                self.mousebox[0][0] : self.mousebox[1][0],
            ]
            ...
            trackSettings.clickpixel = self.img[
                trackSettings.mousecoords[1], trackSettings.mousecoords[0]
            ]
    ...
    def prepare_img_for_tkinter(self):
        if self.collect_images is True:
            ...
            if trackSettings.cameratype == "Windows":
                ...
                self.camera.set_exposure_time(int(float(self.entryExp.get())*1e6))
                self.img = self.camera.get_image()
                self.img = self.scalehisto(self.img)
                ret = True
            if ret is True:
                ...
                self.mousebox = [
                    (
                        int(trackSettings.mousecoords[0] - (trackSettings.boxSize / 2)),
                        int(trackSettings.mousecoords[1] - (trackSettings.boxSize / 2)),
                    ),
                    (
                        int(trackSettings.mousecoords[0] + (trackSettings.boxSize / 2)),
                        int(trackSettings.mousecoords[1] + (trackSettings.boxSize / 2)),
                ...
                self.centerbox = [
                    (
                        int(trackSettings.mainviewX - 5),
                        int(trackSettings.mainviewY - 5),
                    ),
                    (
                        int(trackSettings.mainviewX + 5),
                        int(trackSettings.mainviewY + 5),
                ...
                self.crosshairbox = [
                    (
                        int(trackSettings.crosshairX - 6),
                        int(trackSettings.crosshairY - 6),
                    ),
                    (
                        int(trackSettings.crosshairX + 6),
                        int(trackSettings.crosshairY + 6),
                ...
                self.imgtk = self.img.copy()
                cv2.rectangle(
                    self.imgtk, self.mousebox[0], self.mousebox[1], (255, 0, 0), 2
                )
                cv2.rectangle(
                    self.imgtk, self.centerbox[0], self.centerbox[1], (0, 0, 255), 1
                )
                cv2.circle(
                    self.imgtk,
                    (trackSettings.crosshairX, trackSettings.crosshairY),
                    8,
                    (255, 0, 255),
                    1,
                )
                if trackSettings.objectfollow is True:
                    ...
                    self.roibox, self.imageroi = videotrak.get_x_y(
                        self.img, self.roibox, self.imageroi
                    )
                    ...
                    # self.dlast = self.dnow
                    cv2.rectangle(
                        self.imgtk, self.roibox[0], self.roibox[1], (0, 255, 0), 2
                    )
                ...
                if self.recordvideo.get() == 1:
                    if trackSettings.previousrecord == 0:
                        ...
                        self.out = cv2.VideoWriter(
                            str(self.datetimestring + ".mp4"),
                            self.fourcc,
                            30,
                            (self.width, self.height),
                        )
                        self.datetimestring2 = str(
                            self.dnow.strftime("%m-%d-%Y %H:%M:%S.%f")
                        )
